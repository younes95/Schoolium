var page_actions = function(){
    ...
    $(".lockscreen-box .user_signin").on("click",function(){        
        $(".sign-in").show();
        $(this).remove();
        $(".user").hide().find("img").attr("src",path+"assets/images/users/no-image.jpg");
        $(".user").show();
        return false;
    });
    /* END LOCK SCREEN */
...
function panel_refresh(panel,action,callback){        
    if(!panel.hasClass("panel-refreshing")){
        panel.append('<div class="panel-refresh-layer"><img src="'+path+'img/loaders/default.gif"/></div>');
...
function pageLoadingFrame(action,ver){    
    ...
    loader['v1'] = '<div class="page-loading-loader"><img src='+path+'"img/loaders/page-loader.gif"/></div>';
